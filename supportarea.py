...
#     for i in C:
#         if S[i, k] == 1:
#             problem += pl.lpSum((min(propto[j, k] + l[j],propto[i,k] + l[i])-max(propto[j, k],propto[i,k]))
#                                 *(min(beta[j, k] + w[j],beta[i,k] + w[i])-max(beta[j, k],beta[i,k])) 
#                                 for j in C if j != i and S[j, k] == 1 and gamma[j, k] + h[j] > gamma[i, k]
#                                 and min(propto[j, k] + l[j],propto[i,k] + l[i]) > max(propto[j, k],propto[i,k]) 
#                                 and min(beta[j, k] + w[j],beta[i,k] + w[i]) > max(beta[j, k],beta[i,k])) >= 3 * propto[i, k] * beta[i, k]


x1 = pl.LpVariable.dicts("x1", [(i, j, k) for i in C for j in C if j != i for k in V], 0, None, pl.LpContinuous)
x2 = pl.LpVariable.dicts("x2", [(i, j, k) for i in C for j in C if j != i for k in V], 0, None, pl.LpContinuous)
y1 = pl.LpVariable.dicts("y1", [(i, j, k) for i in C for j in C if j != i for k in V], 0, None, pl.LpContinuous)
y2 = pl.LpVariable.dicts("y2", [(i, j, k) for i in C for j in C if j != i for k in V], 0, None, pl.LpContinuous)
d1 = pl.LpVariable.dicts("d1", [(i, j, k) for i in C for j in C if j != i for k in V], 0, 1, pl.LpBinary)
d2 = pl.LpVariable.dicts("d2", [(i, j, k) for i in C for j in C if j != i for k in V], 0, 1, pl.LpBinary)
d3 = pl.LpVariable.dicts("d3", [(i, j, k) for i in C for j in C if j != i for k in V], 0, 1, pl.LpBinary)
d4 = pl.LpVariable.dicts("d4", [(i, j, k) for i in C for j in C if j != i for k in V], 0, 1, pl.LpBinary)

for k in V:
    for i in C:
        for j in C:
            if j != i:
                problem += x1[i, j, k] <= propto[j, k] + l[j-1]
                problem += x1[i, j, k] <= propto[i, k] + l[i-1]
                problem += x1[i, j, k] >= propto[j, k] + l[j-1] - M * (1- d1[i, j, k])
                problem += x1[i, j, k] >= propto[i, k] + l[i-1] - M * d1[i, j, k]
                problem += x2[i, j, k] >= propto[j, k]
                problem += x2[i, j, k] >= propto[i, k]
                problem += x2[i, j, k] <= propto[j, k] + M * (1- d2[i, j, k])
                problem += x2[i, j, k] <= propto[i, k] + M * d2[i, j, k]
                problem += y1[i, j, k] <= propto[j, k] + l[j-1]
                problem += y1[i, j, k] <= propto[i, k] + l[i-1]
                problem += y1[i, j, k] >= propto[j, k] + l[j-1] - M * (1- d3[i, j, k])
                problem += y1[i, j, k] >= propto[i, k] + l[i-1] - M * d3[i, j, k]
                problem += y2[i, j, k] >= propto[j, k]
                problem += y2[i, j, k] >= propto[i, k]
                problem += y2[i, j, k] <= propto[j, k] + M * (1- d4[i, j, k])
                problem += y2[i, j, k] <= propto[i, k] + M * d4[i, j, k]
...
for k in V:
    for i in C:
        if S[i, k] == 1:
            problem += pl.lpSum((x1[i, j, k]-x2[i, j, k])
                                *(y1[i, j, k]-y2[i, j, k]) 
                                for j in C if j != i and S[j, k] == 1 ) >= 3 * propto[i, k] * beta[i, k]


# for k in V:
#     for i in C:
#         for j in C:
#             if j != i and S[i, k] == 1 and S[j, k] == 1 and min(propto[j, k] + l[j], propto[i,k] + l[i]) > max(propto[j, k], propto[i,k]) and min(beta[j, k] + w[j], beta[i,k] + w[i]) > max(beta[j, k], beta[i,k]):
#                 # 添加约束条件
#                 problem += pl.lpSum((min(propto[j, k] + l[j], propto[i,k] + l[i]) - max(propto[j, k], propto[i,k])) * (min(beta[j, k] + w[j], beta[i,k] + w[i]) - max(beta[j, k], beta[i,k])) >= 3 * propto[i, k] * beta[i, k])
...
if pl.LpStatus[problem.status] == 'Optimal':
    ...
    for k in V:
        ...
        for i in N[:-1]:
            for j in N[1:]:
                if j != i and x[i, j, k].varValue > 0.5:
                    route.append((i, j))
        print(f"Route: {route}")
        ...
        for i in C:
            for m in C:
                if m != i:
                    for j in C:
                        if j != i and j != m and y[i, m, j, k].varValue > 0.5:
                            route2.append((i, m, j))
