...
# 6-15 22：38 通过画图发现修改后的后进先出还是有些问题，只需要对x轴和y轴进行约束即可

# 6-16 10:29 通过求解发现如果不对z轴进行约束会出现先出的在后出的垂直下方，再次修改后进先出约束，借助z坐标是否为0进行约束
...
# eta = model.addVars([(f, i, j, k) for f in range(1, 3) for i in C for j in C if j != i for k in V], vtype=GRB.BINARY)
eta = model.addVars([(f, i, j, k) for f in range(1, 10) for i in C for j in C if j != i for k in V], vtype=GRB.BINARY)
# 定义目标函数
...
# 6-16 10:29 用求并集的思想去修改 引入三个二进制变量，然后使用大M方法修改约束
concentrate11 = model.addVars([(i, j, k) for i in C for j in C if j != i for k in V], vtype=GRB.BINARY)
concentrate12 = model.addVars([(i, j, k) for i in C for j in C if j != i for k in V], vtype=GRB.BINARY)
concentrate13 = model.addVars([(i, j, k) for i in C for j in C if j != i for k in V], vtype=GRB.BINARY)
concentrate21 = model.addVars([(i, j, k) for i in C for j in C if j != i for k in V], vtype=GRB.BINARY)
concentrate22 = model.addVars([(i, j, k) for i in C for j in C if j != i for k in V], vtype=GRB.BINARY)
concentrate23 = model.addVars([(i, j, k) for i in C for j in C if j != i for k in V], vtype=GRB.BINARY)
concentrate31 = model.addVars([(i, j, k) for i in C for j in C if j != i for k in V], vtype=GRB.BINARY)
concentrate32 = model.addVars([(i, j, k) for i in C for j in C if j != i for k in V], vtype=GRB.BINARY)
concentrate33 = model.addVars([(i, j, k) for i in C for j in C if j != i for k in V], vtype=GRB.BINARY)
concentrate41 = model.addVars([(i, j, k) for i in C for j in C if j != i for k in V], vtype=GRB.BINARY)
concentrate42 = model.addVars([(i, j, k) for i in C for j in C if j != i for k in V], vtype=GRB.BINARY)
concentrate43 = model.addVars([(i, j, k) for i in C for j in C if j != i for k in V], vtype=GRB.BINARY)
for k in V:
    for i in C:
        for j in C:
            if j != i:
                ...
                model.addConstr(concentrate11[i, j, k] + concentrate12[i, j, k] + concentrate13[i, j, k] == 1)
                model.addConstr(concentrate21[i, j, k] + concentrate22[i, j, k] + concentrate23[i, j, k] == 1)
                model.addConstr(concentrate31[i, j, k] + concentrate32[i, j, k] + concentrate33[i, j, k] == 1)
                model.addConstr(concentrate41[i, j, k] + concentrate42[i, j, k] + concentrate43[i, j, k] == 1)
                # 以下三选一
                # x[i] >= x[j], y[i] >= y[j], z[i] >= z[j]
                model.addConstr(propto[i, k] - propto[j, k] >= 0 - M * eta[1, i, j, k])
                model.addConstr(beta[i, k] - beta[j, k] >= 0 - M * eta[2, i, j, k])
                model.addConstr(gamma[i, k] - gamma[j, k] >= 0 - M * eta[3, i, j, k])
                # x[i] >= x[j], y[i] >= y[j] + w[j], z[i] >= 0
                model.addConstr(propto[i, k] - propto[j, k] >= 0 - M * eta[4, i, j, k])
                model.addConstr(beta[i, k] - beta[j, k] >= w[j-1] - M * eta[5, i, j, k])
                model.addConstr(gamma[i, k] >= 0 - M * eta[6, i, j, k] )
                # x[i] >= x[j] + l[j], y[i] >= 0, z[i] >= 0
                model.addConstr(propto[i, k] - propto[j, k] >= l[j-1] - M * eta[7, i, j, k])
                model.addConstr(beta[i, k] >= 0 - M * eta[8, i, j, k])
                model.addConstr(gamma[i, k] >= 0 - M * eta[9, i, j, k])

                model.addConstr(gp.quicksum(eta[f, i, j, k] for f in range(1, 4)) <= 3 * (1 - x[i, j, k]) + M * (1 - concentrate11[i, j, k]))
                model.addConstr(gp.quicksum(eta[f, i, j, k] for f in range(4, 7)) <= 3 * (1 - x[i, j, k]) + M * (1 - concentrate12[i, j, k]))
                model.addConstr(gp.quicksum(eta[f, i, j, k] for f in range(7, 10)) <= 3 * (1 - x[i, j, k]) + M * (1 - concentrate13[i, j, k]))
            ...
            if m != i:
                for j in C:
                    if j != i and j != m:
                        ...
                        model.addConstr(gp.quicksum(eta[f, i, j, k] for f in range(1, 4)) <= 3 * (1 - y[i, j, m, k]) + M * (1 - concentrate21[i, j, k]))
                        model.addConstr(gp.quicksum(eta[f, i, j, k] for f in range(4, 7)) <= 3 * (1 - y[i, j, m, k]) + M * (1 - concentrate22[i, j, k]))
                        model.addConstr(gp.quicksum(eta[f, i, j, k] for f in range(7, 10)) <= 3 * (1 - y[i, j, m, k]) + M * (1 - concentrate23[i, j, k]))
                        model.addConstr(gp.quicksum(eta[f, i, j, k] for f in range(1, 4)) <= 3 * (1 - y[m, i, j, k]) + M * (1 - concentrate31[i, j, k]))
                        model.addConstr(gp.quicksum(eta[f, i, j, k] for f in range(4, 7)) <= 3 * (1 - y[m, i, j, k]) + M * (1 - concentrate32[i, j, k]))
                        model.addConstr(gp.quicksum(eta[f, i, j, k] for f in range(7, 10)) <= 3 * (1 - y[m, i, j, k]) + M * (1 - concentrate33[i, j, k]))
                    ...
                    if j != i and j != m:
                        for i_ in C:
                            if i_ != i and i_ != m and i_ != j:
                                ...
                                model.addConstr(gp.quicksum(eta[f, i, j, k] for f in range(1, 4)) <= 3 * (2 - y[m, i, i_, k] - x[m, j, k]) + M * (1 - concentrate41[i, j, k]))
                                model.addConstr(gp.quicksum(eta[f, i, j, k] for f in range(4, 7)) <= 3 * (2 - y[m, i, i_, k] - x[m, j, k]) + M * (1 - concentrate42[i, j, k]))
                                model.addConstr(gp.quicksum(eta[f, i, j, k] for f in range(7, 10)) <= 3 * (2 - y[m, i, i_, k] - x[m, j, k]) + M * (1 - concentrate43[i, j, k]))
