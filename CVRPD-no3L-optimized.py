...
#         problem += propto[i, k] - L_i[i] <= (1 - S[i, k]) * M
# # \beta_{ik}-W+w_i\le\left(1-S_i^k\right)M{\ }\forall i\in C,\forall k\in V
# for k in range(z):
#     for i in range(1, n + 1):
#         problem += beta[i, k] - W_i[i] <= (1 - S[i, k]) * M
# # \gamma_{ik}-H+h_i\le\left(1-S_i^k\right)M{\ }\forall i\in C,\forall k\in V
# for k in range(z):
#     for i in range(1, n + 1):
#         problem += gamma[i, k] - H_i[i] <= (1 - S[i, k]) * M
# # （16）防止同一车辆内的两个箱子重叠：
# # 注：物品i,j在x轴上的投影：i:[cx_{ik},cx_{ik}+l_i],j:[cx_{ik},cx_{ik}+l_i]
# # \propto_{jk}+l_j\le\propto_{ik}+T_{1ijk}M{\ }\forall i,j\in C,j\neq i,\forall k\in V
# for k in range(z):
#     for i in range(1, n + 1):
#         for j in range(1, n + 1):
#             if j != i:
#                 problem += propto[j, k] + L_j[j] <= propto[i, k] + T_1 + t_ij[i, j] * M
# # \propto_{ik}+l_i\le\propto_{jk}+T_{2ijk}M{\ }\forall i,j\in C,j\neq i,\forall k\in V
# for k in range(z):
#     for i in range(1, n + 1):
#         for j in range(1, n + 1):
#             if j != i:
#                 problem += propto[i, k] + L_i[i] <= propto[j, k] + T_2 + t_ij[i, j] * M
# # \beta_{jk}+w_j\le\beta_{ik}+T_{3irk}M{\ }\forall i,j\in C,j\neq i,\forall k\in V
# for k in range(z):
#     for i in range(1, n + 1):
#         for j in range(1, n + 1):
#             if j != i:
#                 problem += beta[j, k] + W_j[j] <= beta[i, k] + T_3 + t_ij[i, j] * M
# # \beta_{ik}+w_i\le\beta_{jk}+T_{4ijk}M{\ }\forall i,j\in C,j\neq i,\forall k\in V
# for k in range(z):
#     for i in range(1, n + 1):
#         for j in range(1, n + 1):
#             if j != i:
#                 problem += beta[i, k] + W_i[i] <= beta[j, k] + T_4 + t_ij[i, j] * M
# # \gamma_{jk}+h_j\le\gamma_{ik}+T_{5ijk}M{\ }\forall i,j\in C,j\neq i,\forall k\in V
# for k in range(z):
#     for i in range(1, n + 1):
#         for j in range(1, n + 1):
#             if j != i:
#                 problem += gamma[j, k] + H_j[j] <= gamma[i, k] + T_5 + t_ij[i, j] * M
# # \gamma_{ik}+h_i\le\gamma_{jk}+T_{6ijk}M{\ }\forall i,j\in C,j\neq i,\forall k\in V
# for k in range(z):
#     for i in range(1, n + 1):
#         for j in range(1, n + 1):
#             if j != i:
#                 problem += gamma[i, k] + H_i[i] <= gamma[j, k] + T_6 + t_ij[i, j] * M
...
# # \propto_{ik}-\propto_{jk}\geq l_j-MQ_{1ij}^k{\ }\forall i,j\in C,j\neq i,\forall k\in V
# for k in range(z):
#     for i in range(1, n + 1):
#         for j in range(1, n + 1):
#             problem += propto[i, k] - propto[j, k] >= L_j[j] - M * Q_1[i, j, k]
# # \beta_{ik}-\beta_{jk}\geq w_j-MQ_{2ij}^k{\ }\forall i,j\in C,j\neq i,\forall k\in V
# for k in range(z):
#     for i in range(1, n + 1):
#         for j in range(1, n + 1):
#             problem += beta[i, k] - beta[j, k] >= W_j[j] - M * Q_2[i, j, k]
# # \gamma_{ik}-\gamma_{jk}\geq h_j-MQ_{3ij}^k{\ }\forall i,j\in C,j\neq i,\forall k\in V
# for k in range(z):
#     for i in range(1, n + 1):
#         for j in range(1, n + 1):
#             problem += gamma[i, k] - gamma[j, k] >= H_j[j] - M * Q_3[i, j, k]
...
#                     if m != i and m != j and j != i and i_ != i and i_ != m and i_ != j:
