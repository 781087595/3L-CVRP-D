...
# 6-13 10:33修改
propto = model.addVars([(i, k) for i in C for k in V], lb=0, ub=L-min(l), vtype=GRB.CONTINUOUS)
beta = model.addVars([(i, k) for i in C for k in V], lb=0, ub=W-min(w), vtype=GRB.CONTINUOUS)
gamma = model.addVars([(i, k) for i in C for k in V], lb=0, ub=H-min(h), vtype=GRB.CONTINUOUS)
...
# （18）不可悬空放置：
# 用辅助变量计算面积--可行性比较高
# 目前存在两个问题：1-支撑面积达不到设置但是显示可行 2-有些坐标为小数位数很多的小数
eps = 0.000001
...
for k in V:
    for i in C:
        ...
        model.addConstr(gamma[i, k] >= 0 + eps - M * (1 - if_gamma0[i, k]) - M * (1 - S[i, k]))
        # model.addConstr(gamma[i, k] >= 0 - M * (1 - if_gamma0[i, k]))
        model.addConstr(gamma[i, k] <= 0 + M * if_gamma0[i, k] + M * (1 - S[i, k]))
...
for k in V:
    for i in C:
        model.addConstr(gp.quicksum(area2[i, j, k] for j in C if j != i) 
                        >= 0.6 * l[i-1] * w[i-1] - M * (1- S[i, k]) - M *(1- if_gamma0[i, k])) 
