...
# for k in V:
#     for i in C:
        # model.addConstr(gp.quicksum((min(propto[j, k] + l[j],propto[i,k] + l[i])-max(propto[j, k],propto[i,k]))
        #                     *(min(beta[j, k] + w[j],beta[i,k] + w[i])-max(beta[j, k],beta[i,k]))
        #                     for j in C if j != i) >= 0.75 * propto[i, k] * beta[i, k] - M * (1 - S[i, k]))
# for k in V:
#     for i in C:
#         model.addConstr(gp.quicksum((gp.min_(propto[j, k] + l[j],propto[i,k] + l[i])-gp.max_(propto[j, k],propto[i,k]))
#                             *(gp.min_(beta[j, k] + w[j],beta[i,k] + w[i])-gp.max_(beta[j, k],beta[i,k]))
#                             for j in C if j != i) >= 0.75 * propto[i, k] * beta[i, k] - M * (1 - S[i, k]))
model.addConstr(gp.quicksum((gp.min_(propto[j, k] + l[j-1],propto[i,k] + l[i-1]).getValue() 
                             - gp.max_(propto[j, k],propto[i,k]).getValue())
